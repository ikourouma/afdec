...
import { getSectorPage, type PageDef, type PageSection } from "@/lib/all-pages-data";
...
import {
  ...
  GraduationCap, ShieldCheck, Heart, ChevronRight, Building2
} from "lucide-react";
...
const accentMap: Record<string, { border: string; text: string; bg: string; badge: string; btn: string; stat: string; dim: string }> = {
  green:   { border: "border-emerald-500/50", text: "text-emerald-400", bg: "bg-emerald-600/10", badge: "bg-emerald-900/30 text-emerald-400 border-emerald-800", btn: "bg-emerald-600 hover:bg-emerald-700", stat: "text-emerald-400", dim: "bg-emerald-600/5" },
  ...
  red:     { border: "border-red-500/50",     text: "text-red-400",     bg: "bg-red-600/10",     badge: "bg-red-900/30 text-red-400 border-red-800",     btn: "bg-red-600 hover:bg-red-700",     stat: "text-red-400",     dim: "bg-red-600/5" },
};

function TextColumnsSection({ section, colors }: { section: PageSection; colors: typeof accentMap["blue"] }) {
...
function HeroStatsSection({ section, colors }: { section: PageSection; colors: typeof accentMap["blue"] }) {
...
function PipelineGridSection({ section, colors }: { section: PageSection; colors: typeof accentMap["blue"] }) {
...
function CtaSection({ section, colors }: { section: PageSection; colors: typeof accentMap["blue"] }) {
...
export default function SectorPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useGSAP(() => {
    ...
    gsap.fromTo(".sector-section",
      { y: 40, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.8, stagger: 0.15, ease: "power2.out",
        scrollTrigger: { trigger: ".sector-content", start: "top 90%" } }
    );
  }, { scope: undefined });
  ...
  return (
    <div className="min-h-screen bg-zinc-950 font-sans selection:bg-blue-500/30">
      ...
      <Breadcrumb />
      <SideNav sections={page.sections} accentColor={page.accent as any} />

      {/* ── Cinematic Hero ── */}
      <section className="relative bg-zinc-950 border-b border-zinc-800/50 overflow-hidden">
        <div className="absolute inset-0 opacity-[0.4]" style={{ backgroundImage: `url('${page.hero_image}')`, backgroundSize: "cover", backgroundPosition: page.hero_image_position ?? "center" }} />
        ...
        <div className="max-w-[1600px] mx-auto px-6 lg:px-12 py-28 md:py-36 relative z-10" ref={heroRef}>
          <div className="flex items-center space-x-2 text-xs font-bold text-zinc-600 uppercase tracking-widest mb-8">
            ...
            <Link href="/sectors" className="hover:text-zinc-400 transition-colors">Sector Intelligence</Link>
            <ChevronRight className="w-3 h-3" />
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link href="/contact" className={`inline-flex items-center px-8 py-4 ${colors.btn} text-white font-black text-sm uppercase tracking-widest rounded-sm transition-all shadow-lg group`}>
              Engage This Sector <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
            </Link>
            <Link href="/sectors" className="inline-flex items-center px-8 py-4 bg-zinc-800/80 hover:bg-zinc-700 text-white font-bold text-sm uppercase tracking-widest rounded-sm transition-all backdrop-blur-sm">
              ← All Sectors
            </Link>
          </div>
        </div>
      ...
      {/* ── Section Dispatcher ── */}
      <main className="sector-content max-w-[1600px] mx-auto px-6 lg:px-12">
        {page.sections.map((section) => (
          <div key={section.id} id={`section-${section.id}`} className="sector-section">
      ...
      <MapTeaser
        heading={`${page.label} Markets Across Africa — Live Data`}
        subtext={`Identify the highest-opportunity African markets for ${page.label.toLowerCase()} using AfDEC’s live economic intelligence. GDP, sector activity, and FDI flows across all 54 nations.`}
        ctaLabel="Open Africa Intelligence Terminal"
        accentColor={(page.accent === "blue" || page.accent === "emerald" || page.accent === "amber" || page.accent === "purple") ? page.accent as "blue" | "emerald" | "amber" | "purple" : "blue"}
      />
      <Newsletter />
      <Footer />
    </div>
